...
- **What the investor likely recomputes on**: the applicable convention an investor measures the number against. Name the basis, timing, and cohort of the codified convention (Standards Board formula or origin author), the standard's default when no specific investor convention is on the table. This is read off the codified definition and described, not predicted.
