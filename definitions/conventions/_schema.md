...
authority: cheatsheet | sec-evidence | standards-board | origin-author | working-consensus
```
...
- **What the investor likely recomputes on**: the plurality or peer-set convention, stated as a hypothesis (public filers are a proxy for the early-stage investor, not direct evidence).
...
- **Citations (REFERENCE, not recall)**: verbatim filer quotes and SEC URLs, standards-board formula, distribution stats with their caveats. All pointers into the evidence doc and cheatsheet.
...
- Never write a card that asserts a contested convention as settled. Distribution percentages and verbatim quotes stay REFERENCE; cards teach that the metric is contested and which axis diverges, not a memorized number.
