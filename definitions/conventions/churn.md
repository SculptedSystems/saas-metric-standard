...
- **The reference's default (use when no specific investor convention is known):** the churn family stated (logo or revenue, gross or net) before any number, with monthly churn annualized by compounding, not by multiplying by twelve. This is the codified read, the defensible default.
...
$$
Primary source: the Standards Board codifies the count-based metric as logo retention, "# of customers at the end of the period who were customers at the beginning of the period / # of active customers at the beginning of the measurement period," recorded in [the reference's verified evidence set](../evidence/canonical-verified.json), sourced from the SaaS Metrics Standards Board's [logo retention](https://www.saasmetricsboard.com/logo-retention) page; churn is one minus that retention. Gross and net revenue churn are the inverses of the codified GRR and NRR entries in the same evidence set (1 - GRR and 1 - NRR). The codified retention definitions are the authority here.
