...
1. **SEC-disclosed definitions from public comparables.** The *primary-filing* layer. These are non-GAAP operating metrics: the SEC expects a key metric to be defined clearly and consistently (2020 MD&A key-metrics interpretive release) but does not prescribe a standard, so this is the most authoritative *public* evidence, not an enforced definition. It is the comparison set investors benchmark a founder against. Built empirically from a saturated frame of public-filer 10-K definitions (321 companies, complete with respect to the tested vocabulary; see [`../evidence/sec-saas-metric-definitions.md`](../evidence/sec-saas-metric-definitions.md) and [methodology](../evidence/methodology.md)). Used for the metrics filers actually disclose: ARR, NRR, GRR.
...
The highest-divergence metric, and the one this sheet is built to win. Public filers split: a recurring-revenue basis is the plurality (about 63% of definers), but recognizable peers diverge, not just obscure filers. Autodesk, Asana, DigitalOcean and Bentley report on a GAAP-revenue basis; Okta, Cloudflare and ZoomInfo on an ACV or annualized basis. In the 321-company frame, 144 filings full-text-match an NRR-family phrase; a stated method could be confirmed for at least 81 (a floor, since regex extraction misses some definitions buried in tables). Among the definitions, the measurement basis diverges (two different-vendor models agree on the basis in about 83% of cases, cross-vendor Cohen's kappa 0.66 for recurring-vs-other: moderate, so the divergence is firm but the precise split is suggestive; a human validation is the next step): some compute it on a recurring-revenue (ARR/MRR) basis, others on GAAP revenue (trailing-twelve-month or calendar), others on contracted/bookings amounts. The time-anchor convention (point-in-time vs an average vs year-over-year) is disclosed so inconsistently that two independent readings cannot classify it the same way, which is itself the point. See [the dataset](../evidence/sec-saas-metric-definitions.md) and [methodology](../evidence/methodology.md).
...
- **SEC filings dataset** (primary): [`../evidence/sec-saas-metric-definitions.md`](../evidence/sec-saas-metric-definitions.md), a saturated frame of 321 public-filer 10-Ks (complete with respect to the tested vocabulary) with verbatim, quote-verified metric definitions; method, completeness evidence, and validation in [`../evidence/methodology.md`](../evidence/methodology.md). Reproducible via `../evidence/sec-pull.js` / `sec-classify.js` / `sec-analyze.js` (EDGAR full-text search + primary-document resolution + LLM classification). See also the [authority discovery memo](../../research/2026-06-16-saas-metrics-authority-discovery.md).
